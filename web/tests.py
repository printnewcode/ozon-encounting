...
class ExportTests(TestCase):
    ...
    def test_sales_report_filters_by_period(self):
        ...
        self.assertIn(sheet['J2'].value, (None, ''))

    def test_sales_report_exports_gross_price_expenses_and_accrual_fields(self):
        product = Product.objects.create(
            article='SKU-1',
            name='Тестовый товар',
            quantity=1,
        ...
        self.assertEqual(sheet['G1'].value, 'Прибыль')
        self.assertEqual(sheet['I1'].value, 'Дата начисления')
        self.assertEqual(sheet['J1'].value, 'ID начисления')
        self.assertEqual(sheet['E2'].value, 220)
        self.assertEqual(sheet['F2'].value, 60)
        self.assertEqual(sheet['G2'].value, 35)
        self.assertEqual(sheet['H2'].value.date(), date(2026, 4, 10))
        self.assertEqual(sheet['I2'].value.date(), date(2026, 4, 12))
        self.assertEqual(sheet['J2'].value, '987654')
